...
def main():
    ...
    parser.add_argument(
        "action",
        help="Action to perform (publish, promote, cleanup, restore, dump, purge)",
    )
    ...
    group_common.add_argument(
        "--url", required=True, help="URL to Aptly API, eg. http://localhost:8080"
    )
    group_common.add_argument(
        "--recreate",
        action="store_true",
    ...
    group_common.add_argument(
        "--no-recreate",
        action="store_true",
        help="Never recreate publish (even when we are adding new components where it's the only option)",
    )
    group_common.add_argument(
        "--force-overwrite",
        action="store_true",
    ...
    group_common.add_argument(
        ...
        action="store_true",
        default=False,
        help="Publish contents. It's slow so disabled by default to support large repositories.",
    )
    group_common.add_argument(
        "--acquire-by-hash",
        action="store_true",
        default=False,
        help="Use Acquire-by-hash option. This may help with repository consistency.",
    )
    group_common.add_argument(
        "--components",
        nargs="+",
        help="Space-separated list of components to promote or restore or to purge (in case of purge)",
    )
    group_common.add_argument(
        "--storage",
        default="",
        help="Storage backend to use for all publishes, can be empty (filesystem, default), swift:[name] or s3:[name]",
    )
    group_common.add_argument(
        "-p", "--publish", nargs="+", help="Space-separated list of publish"
    )
    group_common.add_argument(
        "--gpg-passphrase-file",
        type=str,
        help="The path to the gpg passphrase file for signing a publish",
    )
    ...
    group_publish.add_argument(
        "--dists",
        nargs="+",
        help="Space-separated list of distribution to work with (including prefix), default all.",
    )
    group_publish.add_argument(
        "--architectures",
        nargs="+",
        help="List of architectures to publish (also determined by config, defaults to amd64, i386)",
    )
    group_publish.add_argument(
        "--only-latest",
        action="store_true",
        default=False,
        help="Publish only latest packages of every publishes",
    )

    group_promote = parser.add_argument_group("Action 'promote'")
    group_promote.add_argument(
        "--source",
        help="Source publish to take snapshots from. Can be regular expression, eg. jessie(/?.*)/nightly",
    )
    group_promote.add_argument(
        "--target",
        help="Target publish to update. Must be format if source is regex, eg. jessie{0}/testing",
    )
    group_promote.add_argument(
        "--packages", nargs="+", help="Space-separated list of packages to promote"
    )
    group_promote.add_argument(
        "--diff",
        action="store_true",
        help="Show differences between publishes (snapshots to be updated)",
    )

    group_purge = parser.add_argument_group("Purge")
    group_purge.add_argument(
        "--hard",
        action="store_true",
        default=False,
        help="Remove all unused packages and snapshots",
    )

    group_restore = parser.add_argument_group("Action 'restore'")
    group_restore.add_argument(
        "-r", "--restore-file", help="File used to restore publish"
    )
    ...
    group_save.add_argument(
        ...
        "--save-dir",
        default=".",
        help="Path of where dump of publish will be done",
    )
    group_save.add_argument(
        "-x", "--prefix", default="saved-", help="Prefix for dump files' names"
    )
...
def promote(
    ...
    if not diffs:
        lg.warn(
            "Target {0} is up to date with source publish {1}".format(target, source)
        )
        if not recreate:
    ...
    if packages:
        ...
        for component, snapshots in publish_source.components.items():
            ...
            package_refs = publish_source.get_packages(
            ...
            if package_refs:
                ...
                client.do_post(
                    ...
                    data={
                        ...
                        "Description": "Promoted packages from snapshots %s: %s"
                        % (snapshots, packages),
...
def find_publishes(client, source, target):
    ...
    if not re.search(r"{[0-9]+}", target):
        lg.error(
            "Source publish is regular expression but target does not refer any match groups. See help for more info."
        )
        sys.exit(1)
    lg.debug(
        "Looking for source publishes matching regular expression: {0}".format(source)
    )
    ...
    for publish in publishes:
        ...
        if match:
            try:
                ...
                lg.error(
                    "Can't format target publish {0} using groups {1}".format(
                        target, match.groups()
                    )
                )
                sys.exit(1)
...
def action_promote(client, **kwargs):
    ...
    if re.search(r"\(.*\)", kwargs["source"]):
        for publish in find_publishes(client, kwargs["source"], kwargs["target"]):
            ...
            target = publish[1]
            lg.info(
                "Found source publish matching regex, promoting {0} to {1}".format(
                    source, target
                )
            )
...
def action_diff(source, target, components=[], packages=True):
    ...
    if not diff:
        print(
            "Target {0} is up to date with source publish {1}".format(
                target.full_name.replace("_", "/"), source.full_name.replace("_", "/")
            )
        )
        return
    ...
    for component, snapshots in diff.items():
        ...
        published_source = [
            i for i in source.publish_snapshots if i["Component"] == component
        ][0]["Name"]
        published_target = [
            i for i in target.publish_snapshots if i["Component"] == component
        ][0]["Name"]

        print(
            "\033[1;33m== %s \033[1;30m(%s -> %s)\033[m"
            % (component, published_target, published_source)
        )
        ...
        if packages:
            ...
            diff_packages = source.client.do_get(
                "/snapshots/%s/diff/%s" % (published_source, published_target)
            )
